...
public class ReefMoveAndLift {
    ...
    private final Transform2d tagTorobotLeftStalkPosiion = new Transform2d(0,1, new Rotation2d(0));
    private final Transform2d tagTorobotRightStalkPosiion = new Transform2d(0,0, new Rotation2d(0));
    /**
    ...
    public Command createCommand(int stalk, int level, LadderSubsystem ladder, DriveSubsystem drive, Intake intake){
        if(drive.m_vision.cameraConnected){
            var robotToTag = drive.m_vision.getReefTransform();
            if(robotToTag != null){
                //Left Stalk
                //L2
                if(stalk == 1 && level == 1){
                    var robotToPosition = tagTorobotLeftStalkPosiion.plus(robotToTag);
                    ...
                    .andThen(new IntakeOut(intake).withTimeout(3));
                ...
                //L2
                    ...
                    var robotToPosition = tagTorobotRightStalkPosiion.plus(robotToTag);
                    returnCommands = pathMaker.createPath(drive, new Pose2d().plus(robotToPosition), List.of(), false)
                    .alongWith(new LadderMoveToPosition(ladder, Constants.LadderConstants.bottomStalkPosition))
                    .andThen(new IntakeOut(intake));
                } 
